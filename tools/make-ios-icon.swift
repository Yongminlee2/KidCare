...
let canvasSize = 1024
// 마스코트를 캔버스 가로세로의 80% 로 그린다. 안드로이드 적응형 아이콘의 "안전 영역"
// (108dp 중 가운데 72dp, 72/108 ≈ 0.667)과는 다른 비율이다 — 그 66.7% 는 런처가
// 아이콘을 원형·둥근네모 등 제각각인 마스크로 잘라내도 잘리지 않는 최소 보장치이고,
// iOS 는 마스크 모양이 시스템이 정한 한 가지(연속 곡선 모서리)로 고정돼 있어 그만큼
// 넉넉히 비울 필요가 없다. 0.8 은 그 iOS 쪽 사정에 맞춰 따로 고른 값이다: 모서리가
// 깎여도 그림이 잘리지 않을 만큼, 그러면서 홈 화면에서 옆 아이콘과 다닥다닥 붙어
// 보일 때도 여백이 있어 보이게.
