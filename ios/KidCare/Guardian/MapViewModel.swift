...
/// 지도 화면이 그릴 상태와, 그 상태를 만드는 읽기를 소유한다. Task 1~3 이 `ChildMapView`
/// 의 `@State` 에 쌓아 둔 것(`상태`·`하루기록`·`오류`·`아이_이름`·`서버기준_지금`,
/// `하루를_읽는다()`)을 그대로 옮긴 것이다 — **이 커밋에서는 동작을 하나도 바꾸지
/// 않는다.** 정본은 안드로이드 `MapTimelineFragment` 의 `load`(:304)/`reload`(:341).
///
/// 옮기는 진짜 이유는 이 화면 자체가 아니라 다음 두 Task 다: Task 5(명령 왕복 —
