...
final class MapViewModel {
    ...
    /// 배터리 설명 팝업 한 줄. **카드도 팝업도 안 잠근다** — 아이폰 아이도 배터리 값을
    /// 실제로 올린다. 다만 "어떤 앱이 얼마나 썼는지는 **안드로이드** 보안 정책상 원격으로
    /// 못 읽는다"는 아이폰 아이를 고른 부모에게 틀린 문장이다(통합 검토 L3 와 같은 결).
    /// 안드로이드 아이가 읽는 문장은 **한 글자도 안 바꾼다**(주인 판정) — 그래서 문구를
    /// 고치지 않고 아이폰 전용 키를 하나 더 둔다. `.unknown` 은 다른 모든 자리와 같이
    /// 안드로이드 문구로 물러난다 — 모르는 아이에게 아이폰이라고 단정하지 않는다.
    var 배터리_설명_키: String.LocalizationValue {
        플랫폼 == .iOS ? "ios_child_battery_info_message" : "map_battery_info_message"
    }
    ...
    func 지금_위치를_확인한다() async {
        ...
        do {
            ...
            let commandId = try await firstToFinish(timeoutMillis: sendTimeout, sleep: sleep) {
    ...
    private func stopLiveTrackingCore(sendCommand: Bool) {
        ...
        Task {
            _ = try? await firstToFinish(timeoutMillis: sendTimeout, sleep: sleep) {
