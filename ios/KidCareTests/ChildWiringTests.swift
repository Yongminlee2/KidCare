...
struct ChildWiringTests {
    ...
    /// `func 이름(...) { ... }` 의 본문. 닫는 중괄호는 그 함수의 들여쓰기(4칸)로 찾는다 —
    /// 이 파일들의 함수는 전부 타입 안의 메서드다.
    private func 함수본문(_ 소스글: String, _ 이름: String) throws -> String {
        let 패턴 = "func " + NSRegularExpression.escapedPattern(for: 이름) + "[\\s\\S]*?\\n    \\}"
        let range = try #require(
            소스글.range(of: 패턴, options: .regularExpression),
            "\(이름) 를 못 찾았다 — 이름이 바뀌었으면 이 검사도 함께 옮겨야 한다")
        return String(소스글[range])
    }

    @Test("로그인이 실패하면 화면이 그 사실을 말한다 — build 의 실패 갈래가 cannotStart 를 부른다 (통합 검토 I3)")
    func 실패는_화면으로() throws {
        let body = try 함수본문(try 소스("ios/KidCare/Child/ChildSession.swift"), "build")
    ...
    func 멈춤_배선() throws {
        let body = try 함수본문(try 소스("ios/KidCare/Child/ChildSession.swift"), "stop")
    ...
    func 차단은_start_에() throws {
        let body = try 함수본문(try 소스("ios/KidCare/Child/ChildSession.swift"), "start(familyId:")
