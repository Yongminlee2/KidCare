...
- **WiFi 원격 제어 — 만들지 않기로 확정**(2026-08-08 사용자 결정). 안드로이드 10부터
  `WifiManager.setWifiEnabled` 는 서드파티 앱에 항상 실패를 돌려준다. 유일한 길인
  기기 소유자(Device Owner) 등록은 **아이 폰 초기화**가 전제인데 사용자가 감수하지
  않기로 했다. `targetSdk` 를 28 로 내리면 옛 API 가 살아나지만 그러면 위치 수집이
  망가진다 — 앱의 본체를 부수는 거래다. 다시 꺼내지 말 것.
