...
families/{familyId}
  ...
  children/{childUid}/segments/{autoId}   (3단계 구현 반영 — 아래로 정정)
    type: "STAY" | "MOVE"                  (대문자로 저장한다)
    startAt, endAt
    lat, lng, distanceMeters, pointCount
    placeName                              (stay 만 채운다. §4.2 참고. move 는 빈 문자열)
    dayKey                                 (그 구간이 시작한 날, 자녀 폰 시간대 기준.
                                            segments 를 찾는 유일한 질의 키라서
                                            반드시 있어야 한다 — startAt 범위로 매번
                                            "이게 몇 일치인가"를 계산하면 자정을 걸친
                                            구간에서 시간대가 바뀔 때 어긋난다)
...
2. ~~`onEventCreated`~~ — `events/` 문서 생성을 보호자 화면이 Firestore 스냅샷
   리스너로 직접 구독한다.
3. ~~`dailyCleanup`~~ — **3단계 Task 8에서 구현 완료.** 매일 04:00(KST) 고정
   대신 대안 (a)를 택했다: 자녀 폰의 `child/PointsCleaner`가 `TrackingService`
   안에서(구간 재계산과 같은 "마지막 실행 시각 비교" 방식으로) 하루 한 번 30일
   지난 `points/`를 배치로 지운다. 정해진 새벽 시각이 아니라 자녀 폰이 살아있는
   동안 아무 때나 하루 한 번 도는 방식이라 원래 계획과는 다르지만, 결과(30일
   뒤 자동 삭제가 실제로 일어남)는 같다. 자세한 내용과 배치 상한은
   `docs/known-issues.md` 항목 4 참고.
...
- 직전 업로드 지점에서 **25m** 이내면 업로드를 생략한다(배터리·통신량·Firestore 사용량).
...
- **이름을 물어볼 좌표는 `Segment.nameLat`/`nameLng`다**(2026-08-07 추가). 지도에 찍는
  ...
  `SegmentDoc`에는 저장하지 않는다(스키마 불변, `firestore.rules` 불변). 하한이 5m 인
  이유는 옛 문서의 `accuracy = 0`이 "완벽"이 아니라 "모른다"인데 1/0² 로 가중치를
  독점해 버리기 때문이다.
