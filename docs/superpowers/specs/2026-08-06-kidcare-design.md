...
- 정확도(accuracy) 100m 초과 지점은 `LocationFilter.decide()`가 업로드하기 전에
  거절한다(`REJECT_INACCURATE`) — Firestore에 아예 저장되지 않으므로 "구간
  계산에서 제외"할 대상 자체가 없고, 지도에 흐리게 표시할 수도 없다(표시할
  데이터가 애초에 안 올라온다). 이전 판은 "지도에는 흐리게 표시한다"고 적었는데
  실제로 그럴 수 있는 경로가 없어 지운다.
