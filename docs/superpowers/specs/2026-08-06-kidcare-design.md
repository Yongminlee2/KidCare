...
Blaze(종량제)를 쓴다. 카드 등록이 필요하지만 가족 3명 규모는 무료 한도의 1% 미만이라
실제 청구액은 월 0원으로 예상된다. 예산 알림을 1,000원에 걸어 안전장치를 둔다.

Spark(무료)로도 동작은 하지만, Cloud Functions를 못 써서 FCM 푸시를 보낼 수 없다.
그러면 자녀 폰이 Firestore 리스너로만 명령을 받게 되는데, 삼성 절전 정책에 앱이 강제
종료되면 "핸드폰 찾기"가 실패한다. 신뢰성이 기능의 존재 이유인 부분이라 Blaze를 택했다.
...
| 결정 | 대안 | 이유 |
|---|---|---|
| Firebase Blaze | Spark 무료 | Spark는 Cloud Functions를 못 써 FCM 푸시가 불가. 삼성 절전에 앱이 죽으면 폰찾기가 실패한다. 가족 규모에선 실제 요금 0원 |
