...
3. **활동 인식에 대응하는 "죽은 앱을 깨우는" 장치가 없다.** CoreMotion 은
   ...
   **이 절이 새로 만드는 유일한 상수다.** 안드로이드에 짝이 없는 상수는 이것
   말고도 셋 더 있는데, 전부 §6.4 가 업로드 주기를 아이폰용으로 다시 정하면서
   도입한 것이다(`uploadMinIntervalMillis` 15분, `uploadIdleIntervalMillis` 4시간,
   `uploadEventMinGapMillis` 60초 — 안드로이드는 `SAFETY_UPLOAD_INTERVAL_MILLIS`
   24시간 하나로 하던 일이다). 그 셋은 §6.4 가 근거이고 코드 주석이 §6.4 를
   인용한다. **짝 없는 상수를 새로 들일 때는 이 문단에 이유와 함께 적는다** —
   여기 없는 짝 없는 상수는 지어낸 값이라는 뜻이다(4단계 통합 검토 L4).
