...
이 작업은 코드로 대신할 수 없다 — 콘솔 게시 버튼은 사람이 직접 눌러야 한다.
...
**`firestore.rules`를 고칠 때마다 매번 다시 게시해야 한다.** 로컬 파일을 바꾸는
것만으로는 아무 효과가 없다 — 콘솔에 저장된 규칙이 실제로 적용되는 규칙이다.
파일이 바뀔 때마다 위 1~3단계(콘솔 접속 → 저장소의 `firestore.rules` 내용 전체를
복사해 편집기에 덮어쓰기 → `게시`)를 다시 밟는다. 반영됐는지 확인하는 방법: 보호자
...
### 6단계는 규칙을 안 바꿨다 — 다시 게시할 필요 없다

`firestore.rules` 는 5단계 이후로 **한 줄도 안 바뀌었다**(2026-08-08 대조).
6단계가 한 일은 앱 쪽 제한시간·문구·아이콘·릴리스 빌드라 규칙과 무관하다.
새 APK 만 깔면 되고 콘솔은 건드리지 않는다.
...
없다(그 컬렉션을 읽는 코드가 없다). **지금 만들어야 하는 색인은 아래 `commands`
하나뿐이다.**
...
### 복합 색인 (4단계 — 명령이 하나도 안 갈 때)

**지금 필요한 색인은 이것 하나다.** 이게 없으면 4단계 이후 기능이 통째로 죽는다.
